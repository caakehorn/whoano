...
export default function SandboxGame({ onExit }: { onExit?: () => void }) {
  ...
  // Game State
  const [levelType, setLevelType] = useState<'SANDBOX' | 'DOWNHILL_30' | 'DYNAMIC_HILLS'>('SANDBOX');
  ...
  useEffect(() => {
    ...
    const engine = Engine.create({ gravity: { x: 0, y: 4.5, scale: 0.001 } });
    ...
    const ball = Bodies.circle(W * 0.1, -H * 0.5, ballRadius, {
      restitution: 0.4, // Less bouncy for downhill
      friction: 0.01, 
      frictionAir: 0.005, 
      density: 0.005, 
      label: 'ball'
    ...
    if (levelType === 'SANDBOX') {
        ...
        for (let i = 0; i < segments; i++) {
            ...
            const circle = Bodies.circle(currX, currY, 30, {
                isStatic: true,
                label: 'surface',
                restitution: 0.1,
                friction: 0.001
    ...
    const loop = (time: number) => {
      ...
      while (accumulator >= timeStep) {
        ...
        if (st.keys['KeyW'] || st.keys['ArrowUp']) dy = -1;
        if (st.keys['KeyS'] || st.keys['ArrowDown']) dy = 1;

        if (isAirborne) {
            // Trick Logic
            let newTrickKeys = '';
            if (dx === 1 && dy === 0) newTrickKeys = 'Grab';
            if (dx === -1 && dy === 0) newTrickKeys = 'Rewind';
            if (dy === -1 && dx === 0) newTrickKeys = 'Spike';
            if (dy === 1 && dx === 0) newTrickKeys = 'Slam';
            if (dx === 1 && dy === -1) newTrickKeys = 'Corkscrew';
            if (dx === 1 && dy === 1) newTrickKeys = 'Method';
            if (dx === -1 && dy === -1) newTrickKeys = 'Rodeo';
            if (dx === -1 && dy === 1) newTrickKeys = 'Backflip';

            if (newTrickKeys !== '') {
                if (!st.currentTrick) {
                    st.currentTrick = newTrickKeys;
                    st.trickStartTime = Date.now();
                } else if (st.currentTrick !== newTrickKeys && Date.now() - st.trickStartTime < 150) {
                    st.currentTrick = newTrickKeys;
                }
            } else {
                if (st.currentTrick) {
            ...
            // Air Control (Mild stabilization or nudging)
            if (dx !== 0) {
                Body.applyForce(ball, ball.position, { x: dx * 0.00015 * ball.mass, y: 0 });
            }
            if (dy > 0) {
                // Mild fast fall
                Body.applyForce(ball, ball.position, { x: 0, y: 0.0005 * ball.mass });
            }
        } else {
            // Grounded
            if (st.currentTrick) {
                // Landed while holding a trick - Bail!
                showNotif('BAIL!', 'TRICK INTERRUPTED', '#ef4444');
                st.currentTrick = null;
                st.chainCount = 0;
                st.streak = 1;
                setStreak(1);
                ...
                // Penalty 
                Body.setVelocity(ball, { x: ball.velocity.x * 0.3, y: ball.velocity.y });
            } else if (st.wasAirborne) {
                // Landed Cleanly
                if (st.streak > 1) {
                    showNotif('COMBO LANDED!', `x${st.streak}`, '#0ea5e9');
                }
                // We keep streak? The prompt said "Score multiplier is purely combo-driven — land tricks without touching ground."
                // This means the multiplier works DURING the air. Touching ground banks combo and resets.
                st.chainCount = 0;
                st.streak = 1;
                setStreak(1);
            }
            
            // Ground manual adjust
            if (dx !== 0) {
                Body.applyForce(ball, ball.position, { x: dx * 0.0001 * ball.mass, y: 0 });
                Body.setAngularVelocity(ball, ball.angularVelocity + dx * 0.02);
            }
        }
